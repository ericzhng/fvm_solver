...
def run_shallow_water_test(n_cells: int = 100, t_final: float = 0.1, snapshot_interval: float = 0.02):
    """Run dam-break test for shallow water equations.
    ...
    equation_system = ShallowWaterSystem(g=9.81, h_min=1e-10)
    ...
    for flux_method in ['HLLC', 'Roe']:
        for recon_method, limiter in configs:
            ...
            print(f"Final height (Dam-Break, {recon_method}, {limiter}, {flux_method}):", U_current[0])

if __name__ == "__main__":
    run_shallow_water_test()