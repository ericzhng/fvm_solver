...
def run_test(equation_system, test_name, n_cells=100, t_final=0.1, snapshot_interval=0.02):
    ...
    for flux_method in ['Lax-Friedrichs', 'Rusanov', 'FORCE', 'HLL', 'HLLC', 'Roe']:
        solver = GodunovSolver(equation_system=equation_system, flux_method=flux_method, limiter='minmod', cfl=cfl, bc_type='transmissive')
        t = 0
        U_current = U.copy()
        ...
        print(f"Final {equation_system.get_variable_names()[0].lower()} ({test_name}, {flux_method}):", U_current[0])
