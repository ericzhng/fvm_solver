...
class Reconstruction:
    """Handles spatial reconstruction for Godunov-type solvers."""
    
    def __init__(self, equation_system: EquationSystem, limiter: str = 'minmod'):
    ...
    def muscl(self, U: np.ndarray, dx: float) -> tuple:
        """MUSCL reconstruction (second-order, piecewise linear)."""
        n = U.shape[1]
        UL = np.zeros_like(U[:, :-1])
        UR = np.zeros_like(U[:, :-1])
    ...
    def ppm(self, U: np.ndarray, dx: float) -> tuple:
        """Piecewise parabolic method (third-order)."""
        n = U.shape[1]
        UL = np.zeros_like(U[:, :-1])
        UR = np.zeros_like(U[:, :-1])
        W = np.array([self.equation_system.to_primitive(U[:, i]) for i in range(n)]).T
    ...
    def weno5(self, U: np.ndarray, dx: float) -> tuple:
        """WENO5 reconstruction (fifth-order)."""
        n = U.shape[1]
        UL = np.zeros_like(U[:, :-1])
        UR = np.zeros_like(U[:, :-1])
        W = np.array([self.equation_system.to_primitive(U[:, i]) for i in range(n)]).T
