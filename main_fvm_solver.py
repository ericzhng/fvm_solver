...
from src.utils import parse_xml_config, create_grid  # Make sure this import path matches your project structure
...
def main():
    ...
    # Set up grid
    grid = create_grid(config['xmin'], config['xmax'], config['nx'], config['stretch_factor'], config['dimension'])
    ...
    W0[:, :split_idx] = np.array(config['initial_conditions']['left'], dtype=float)[:, np.newaxis]
    W0[:, split_idx:] = np.array(config['initial_conditions']['right'], dtype=float)[:, np.newaxis]
    ...
    solver = Solver(
        ...
        cfl=config['cfl'],
        flux=config['flux'],
        str_reconstruction=config['reconstruction'],
    ...
    # Solve and save
    U_history, final_t = solver.solve(U0, config['T'], config['time_integration'])
